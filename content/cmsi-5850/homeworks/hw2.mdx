...
**Antisymmetric** — If $a$ is related to $b$ and $b$ is related to $a$, then $a$ must equal $b$.
...
>
> - **Data structures:** Represent your universe as a set (or list) and your relation as a set of pairs/tuples. Using a hash-set for $R$ makes membership checks $O(1)$ — important since transitivity requires checking many triples.
...
> - **Equivalence and Partial Order** are just conjunctions of the properties above — no new logic needed.
...
**Function** — Every element of $A$ maps to exactly one element of $B$.
...
**Partial Function** — Some elements of $A$ may have no mapping, but those that do map to exactly one element.
...
Output `true` only if both properties hold. A bijection establishes a one-to-one correspondence between $A$ and $B$ — recall from LN 3 that this is how we define "same cardinality."
...
> - **Partial function:** Same as a function but relax condition (1) — some keys may be missing. Check that no key maps to more than one value.
...
- **Abstractions:** `(\x. body)` — the backslash `\` is the ASCII-friendly lambda. Optionally accept the Unicode `λ` as well
- **Applications:** `(f x)` — always fully parenthesized in input files for unambiguous parsing
...
>
> - **Two-phase approach:** Separate *tokenizing* from *parsing*. First, scan the input string into a flat list of tokens: `LPAREN`, `RPAREN`, `LAMBDA` (for `\` or `λ`), `DOT`, `VARIABLE("x")`, `CONSTANT(42)`, etc. Then parse the token list recursively.
> - **AST representation:** Model your abstract syntax tree with four variants — one for each expression form. In an OOP language, use a class hierarchy or tagged objects. In a functional language, use an algebraic data type. For example:
>   - `Var(name)` — a variable
>   - `Const(value)` — a constant
>   - `Abs(param, body)` — an abstraction (param is a string, body is an AST)
>   - `App(func, arg)` — an application (both func and arg are ASTs)
> - **Parsing logic:** Since input is fully parenthesized, parsing is straightforward. When you see `(`, peek at the next tokens: if you see `\` or `λ`, it's an abstraction; otherwise it's an application. Recursively parse the sub-expressions and consume the closing `)`.
...
Implement capture-avoiding substitution $M[N/x]$ — the most subtle part of the Lambda Calculus.
...
| Abstraction $\lambda x. M$ | $\lambda x. M$ | $x$ is bound — leave it alone |
...
**Key test case from lecture:** $(\lambda y. f \; x \; y)[y/x]$ should produce $\lambda z. f \; y \; z$ — *not* $\lambda y. f \; y \; y$ (which would be a capture bug).
...
>   2. If $y \notin FV(N)$ OR $x \notin FV(M)$: safe to proceed — return $\lambda y. M[N/x]$
>   3. Otherwise: pick a fresh $z$ not in $FV(M) \cup FV(N)$, rename $y$ to $z$ in $M$, then substitute — return $\lambda z. (M[z/y])[N/x]$
...
> - **CBN step logic:** Find the leftmost outermost beta-redex — an application `App(Abs(x, body), arg)` — and reduce it immediately via substitution. Do NOT evaluate `arg` first.
...
> - **Built-in evaluation:** When you encounter `App(Const("succ"), Const(n))`, return `Const(n+1)`. For two-argument built-ins like `plus`, you'll see `App(App(Const("plus"), Const(a)), Const(b))` — return `Const(a+b)`. Handle these as special cases in your step function.
...
**`basics.lam`** — Simple expressions testing fundamental operations:
...
The first is $\Omega$ — it should output `[DIVERGES]` under both strategies. The second is $(\lambda x. 0) \; \Omega$ — it should output `[DIVERGES]` under CBV but `0` under CBN. This is the key test from lecture that distinguishes the two strategies.
...
## The Wasm Abstract Machine — A Set-Theoretic Model

This is **Stage 2** of the WebAssembly project. In HW2, you built a relation analyzer and a lambda calculus interpreter — operationalizing set-theoretic definitions and function application. In this optional, you will use those same mathematical tools to formalize how WebAssembly's runtime actually works.

Wasm's specification defines its runtime structures — values, the store, frames, the stack — using the same mathematical notation you studied in Lectures 3 and 4. Your job is to read the spec and express what you find using the formal language of sets, relations, functions, and tuples.

### Spec Sections to Read

1. [**Execution → Runtime Structure**](https://webassembly.github.io/spec/core/exec/runtime.html) — This is the core reading. It defines *values*, the *store*, *addresses*, *module instances*, *function instances*, the *stack*, *labels*, *frames*, and *configurations* — all using mathematical notation.

2. [**Structure → Instructions**](https://webassembly.github.io/spec/core/syntax/instructions.html) — The abstract syntax of instructions (the AST from Stage 1).

3. [**Introduction → Overview → Concepts**](https://webassembly.github.io/spec/core/intro/overview.html) — Re-read the "Functions," "Linear Memory," and "Tables" sections with set-theoretic eyes.

### Questions

<QuestionBox qid="Q1" title="The Wasm Value Domain" points={3}>

The spec defines values as $\mathit{val} ::= \mathit{num} \mid \mathit{vec} \mid \mathit{ref}$. For our `i32`-only subset, the value domain is restricted to <InlineMath formula="\mathit{val} = \lbrace \mathsf{i32.const}\ n \mid n \in \mathbb{Z}_{2^{32}} \rbrace" />.

Express this as a set using the notation from Lecture 3. What is its cardinality? Is it finite, countably infinite, or uncountable? Justify your answer.

</QuestionBox>

<QuestionBox qid="Q2" title="The Store as a Tuple of Functions" points={5}>

The spec defines the store as a record of sequences: $\mathit{store} = (\mathit{funcs}, \mathit{globals}, \mathit{mems}, \mathit{tables})$ where each component maps addresses (natural numbers) to instances.

Using the tuple and function notation from Lecture 3:

<SubQuestion qid="Q2a" points={2}>

Define each component of the store as a function. For example, express $\mathit{funcs}$ as $\mathit{funcs} : \mathbb{N} \to \mathit{funcinst}$. Do the same for $\mathit{globals}$, $\mathit{mems}$, and $\mathit{tables}$.

</SubQuestion>

<SubQuestion qid="Q2b" points={3}>

For each of these functions, determine: is it total or partial? Injective? Surjective? Justify each classification. Consider: can two different addresses point to the same instance? Must every possible natural number be a valid address?

</SubQuestion>

</QuestionBox>

<QuestionBox qid="Q3" title="Frames and Environments" points={5}>

The spec defines $\mathit{frame} = \lbrace \mathsf{locals}\ \mathit{val}^*, \mathsf{module}\ \mathit{moduleinst} \rbrace$.

<SubQuestion qid="Q3a" points={2}>

Express the `locals` component as a function $\mathit{locals} : \mathbb{N} \to \mathit{val}$, where the domain is the set of valid local indices. How does this compare structurally to the environment $\sigma : \text{Identifier} \to \text{Value}$ that you will encounter later in the course (Lecture 10)? What plays the role of "identifier" in Wasm?

</SubQuestion>

<SubQuestion qid="Q3b" points={3}>

In the lambda calculus (HW2 Part 2), environments bind variable names to values during substitution. In Wasm, locals bind integer indices to values during execution. Compare these two binding mechanisms: what is structurally the same? What is fundamentally different about using names vs. indices?

</SubQuestion>

</QuestionBox>

<QuestionBox qid="Q4" title="Tracing a Function Call" points={4}>

Consider a Wasm function that adds two locals:
...
Trace the full state evolution when calling `$add(3, 5)`: frame creation → local binding → instruction execution → result. Then compare each step to beta-reduction in the lambda calculus: $(\lambda x.\lambda y.\ x + y)\ 3\ 5 \to_\beta (\lambda y.\ 3 + y)\ 5 \to_\beta 3 + 5 \to 8$. Where does argument binding happen in each model? Where does evaluation happen? What structure mediates each?

</QuestionBox>

<QuestionBox qid="Q5" title="The Store vs. the Environment" points={3}>

The Wasm store is *global and mutable* — all modules share it and can modify it. The lambda calculus has no mutable state; every application produces a fresh substitution.

Using the function and relation definitions from Lecture 3, explain why this difference matters. What property does the lambda calculus have that Wasm does not? (Hint: can you always replace equals with equals in Wasm? Consider what happens when two function calls read from and write to the same global variable.)
