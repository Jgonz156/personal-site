...
## Reading Inference Rules — A Wasm Type Checker

This optional assignment is **Stage 1** of a semester-long project: building pieces of a real [WebAssembly](https://webassembly.github.io/spec/core/) (Wasm) implementation. WebAssembly is a formally specified, industry-standard compilation target that runs in every major browser and an increasing number of server-side environments. Its specification is written using the exact formalisms you are studying in this course — and you are going to read it.

In HW1, you read and reasoned about inference rules and natural deduction proofs on paper. In this optional, you will translate those same structures into **code** — for a real industrial specification.
...
- **Value type:** `i32` only
- **Numeric:** `i32.const`, `i32.add`, `i32.sub`, `i32.mul`, `i32.div_s`, `i32.eq`, `i32.lt_s`, `i32.gt_s`, `i32.eqz`
- **Variables:** `local.get`, `local.set`
- **Control:** `block`, `loop`, `if`/`else`/`end`, `br`, `br_if`, `return`, `nop`
- **Functions:** `call`
- **Parametric:** `drop`

This subset supports factorial, fibonacci, and simple loops — everything you need to see the full formal pipeline at work.

### Spec Sections to Read

Before you begin coding, read the following sections of the [WebAssembly Core Specification](https://webassembly.github.io/spec/core/):

1. [**Introduction → Overview**](https://webassembly.github.io/spec/core/intro/overview.html) — Read the "Semantic Phases" section. Note that Wasm's pipeline is: Decoding → **Validation** → Execution. This stage implements Validation.

2. [**Structure → Types**](https://webassembly.github.io/spec/core/syntax/types.html) — Focus on `numtype` (specifically `i32`), `valtype`, and `functype`. These are the types your checker will work with.

3. [**Validation → Instructions**](https://webassembly.github.io/spec/core/valid/instructions.html) — This is the core reading. Each instruction has a typing rule written as an inference rule. Start with the simple ones:
   - `nop` — an axiom with no premises
   - `drop` — consumes one value of any type
   - `local.get` — reads from the context
   - `i32.add` — consumes two `i32` values, produces one (identical in structure to a BinaryOp rule)

4. [**Validation → Conventions**](https://webassembly.github.io/spec/core/valid/conventions.html) — Understand the typing context $C$. It contains `types`, `funcs`, `locals`, `labels`, and `return`. This is the analog of the environments you will study later in the course.

### Your Task

Implement a function `typecheck(instruction, context)` that takes an instruction AST node and a typing context, and returns the instruction's type signature (input types → output types) or raises a type error.

Your type checker should handle all instructions in the target subset above. For each instruction, your code should correspond directly to one inference rule from the spec. Add a comment above each case citing the spec rule you are implementing (e.g., "See spec: valid/instructions.html § i32.add").

You may use **any programming language**.

### Scaffolding

Below is a reference data model you can use or adapt. You do not need to follow this structure exactly — it is here to get you started.

**AST types** — representing the instructions in the target subset:
...
**Typing context** — the information available during validation:
...
**Type signature** — what your function returns for each instruction:
...
**Test 1 — Simple arithmetic:** Context: `locals = ["i32", "i32"]`. Instructions: `[local.get 0, local.get 1, i32.add]`. Expected signature: `[] → ["i32"]`.

**Test 2 — Type error:** Context: `locals = []`. Instructions: `[local.get 0]`. Expected: type error (index out of bounds).

**Test 3 — Conditional:** Context: `locals = ["i32"]`, `labels = []`. Instructions: `[local.get 0, if (result i32) [i32.const 1] else [i32.const 0] end]`. Expected signature: `[] → ["i32"]`.

**Test 4 — Function call:** Context: `locals = ["i32"]`, `funcs = [{ params: ["i32"], results: ["i32"] }]`. Instructions: `[local.get 0, call 0]`. Expected signature: `[] → ["i32"]`.

**Test 5 — Drop and nop:** Context: `locals = ["i32"]`. Instructions: `[local.get 0, drop, nop]`. Expected signature: `[] → []`.
