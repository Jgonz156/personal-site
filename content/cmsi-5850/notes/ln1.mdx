...
- **What is a Programming Language?** — Defining our field of study
- **The Many Ways to Say One Thing** — Why language diversity matters
- **A Journey Through History** — How languages evolved alongside theory
- **Why Study This?** — Professional, social, and academic benefits
...
But what is computation? It's not just "what computers do"—the concept predates electronic computers by decades.
...
Step 1: Filter evens → [-2, 20, 8, 0, 10]
Step 2: Square each  → [4, 400, 64, 0, 100]
Step 3: Sum them     → 568
```

How many ways can we express this computation? Let's look at just four approaches from different eras:

**Fortran (1957)** — Explicit iteration, close to the machine:
...
**Haskell (1990)** — Point-free functional composition:
...
**Python (1991)** — Readable comprehension syntax:
...
**Rust (2010)** — Safe iterator chains with explicit types:
...
> **🤔 Pause and consider:** These four solutions express the *same* computation. Yet they look radically different. Why? Because each language was designed for a different era, with different priorities, solving different problems.

This course is about understanding **why** these differences exist—not just *how* to write code, but *how languages themselves are designed, formalized, and evaluated*.
...
| 1930s | **Gödel's Incompleteness** | Limits of what can be computed/proven |

These theoretical tools—developed before electronic computers existed—would eventually become the bedrock of programming language design. We'll study lambda calculus in depth in **LN 4-5**.

> **📌 Course Preview:** Throughout this course, you'll see how these 1930s ideas directly influence modern language features. Lambda calculus isn't just history—it's the basis for every function you've ever written.
...
### Fortran (1957) — Scientific Computation
...
Notice the **array operations**—Fortran was designed for numerical computation, so operating on entire arrays is natural.
...
> **🤔 Why so awkward?** COBOL wasn't designed for numerical algorithms—it was designed for **reading records from files, processing transactions, and generating reports**. Asking COBOL to sum squares is like asking a spreadsheet to render 3D graphics. It *can* do it, but that's not what it's *for*.

> **💡 Key Insight:** Each of these languages succeeded because they **matched their syntax to their domain**. Scientific formulas look like Fortran. Symbolic manipulation looks like Lisp. Business logic reads like COBOL—and our algorithm looks awkward in COBOL precisely *because* it's not a business problem.

### 🔬 Foundations Spotlight: BNF Notation

In 1959, John Backus introduced **Backus-Naur Form (BNF)** to formally describe Algol's syntax. This was revolutionary—for the first time, we had a precise, mathematical way to define what constitutes a valid program.
...
### BASIC (1964) — Democratizing Programming

**The Problem:** Programming was only for specialists. How could we make it accessible to students?

**The Solution:** Dartmouth's BASIC—simple, interactive, forgiving.
...
### Simula (1967) — The Seeds of Object-Orientation

**The Problem:** How do we model real-world entities in code?

**The Solution:** Ole-Johan Dahl and Kristen Nygaard created Simula, introducing **classes** and **objects**—concepts that would dominate the next 50 years.
...
This looks similar to other procedural languages—so where's the object-orientation? Simula's innovation wasn't about *this* kind of problem. Its power shines when modeling **simulations**:
...
> **🤔 Why show both?** The sum-of-squares algorithm doesn't *need* objects—it's a numerical computation. But modeling a bank queue with customers, tellers, and waiting lines? That's where Simula's classes shine. The language was designed for **discrete event simulation**, and its OO features emerged from that need.

Simula's `Class` concept would inspire Smalltalk, C++, Java, and virtually every OO language that followed.

### APL (1966) — Radical Expressiveness
...
This single line does everything: filter evens, square, sum. APL traded readability for expressiveness—a tradeoff that sparked decades of debate.

### 🔬 Foundations Spotlight: Denotational Semantics Begins

In the 1960s, Christopher Strachey and Dana Scott began developing **denotational semantics**—a way to define program meaning by mapping syntax to mathematical objects. Rather than describing *how* a program executes step-by-step, denotational semantics describes *what* a program means.
...
> **📌 Why does this matter?** Denotational semantics lets us *prove* things about programs mathematically. If two programs have the same denotation, they're equivalent—regardless of how they execute.
...
### Pascal (1970) and C (1972) — Structured Control Flow
...
Notice the pointer arithmetic—C sits close to the hardware while still providing structured abstractions.

### Smalltalk (1972) — Pure Object-Orientation
...
Notice the radical difference: we don't say *how* to iterate—we declare *what* the answer means:
...
### ML (1973) — Type Inference and Functional Purity

**The Problem:** How do we get the safety of static types without verbose type annotations?

**The Solution:** Robin Milner's ML introduced **type inference**—the compiler figures out types for you.
...
> **💡 Key Insight:** The 1970s gave us four major paradigms: **imperative**, **object-oriented**, **logic**, and **functional**. Most modern languages are hybrids of these approaches.

### 🔬 Foundations Spotlight: Operational Semantics and Type Theory
...
### C++ (1985) and Objective-C (1984) — Hybrid Approaches
...
### Erlang (1986) — Fault-Tolerant Distributed Systems
...
> **💡 Key Insight:** Languages are increasingly designed for **specific industrial needs**. Erlang wasn't trying to be a general-purpose language—it was solving telecom's reliability problem.

### 🔬 Foundations Spotlight: Process Calculi
...
These calculi let us formally prove properties about concurrent programs—essential for systems like Erlang.
...
### JavaScript (1995) — The Browser's Language
...
### Python (1991) and Ruby (1995) — Scripting and Expressiveness
...
### Haskell (1990) — Pure Functional Programming Matures

**The Problem:** How do we build a language around mathematical purity?

**The Solution:** Lazy evaluation, monads, pure functions—a laboratory for PL research.
...
### 🔬 Foundations Spotlight: The Danger of No Formal Spec

> ⚠️ **Cautionary Tale:** JavaScript and PHP were created rapidly without formal specifications. The result? Years of browser incompatibilities, undefined behavior, and security vulnerabilities.
...
**Lesson:** Formal foundations aren't academic luxuries—they prevent real-world chaos.
...
1. **Multicore processors** — Shared mutable state is the enemy of parallelism
2. **Big Data** — Map/reduce is inherently functional

### Scala (2004), Clojure (2007), F# (2005) — Functional Meets Mainstream
...
**The Solution:** Rust's **ownership system**—compile-time enforcement of memory safety.
...
### Swift (2014) and Kotlin (2011) — Modern Successors
...
### 🔬 Foundations Spotlight: Types as Proofs
...
> **🤔 The cycle continues:** Just as Fortran emerged for scientific computation and COBOL for business, new domains spawn new languages. What problems do *we* face today that might require entirely new languages?
...
| **Formalized after the fact** | C (K&R → ANSI), JavaScript (ES5+) | Specification chases implementation |
...
Having traced this history, we can now appreciate why studying programming languages—not just *using* them—provides profound benefits.

### Professional Benefits

| Benefit | How History Shows It |
| :--- | :--- |
| **Express yourself better** | Haskell's point-free style vs. Java's verbosity—knowing both expands your vocabulary |
...
| **Create new languages** | DSLs (Domain-Specific Languages) are everywhere—SQL, regex, CSS |
...
> **📌 The Takeaway:** Every language is a response to the problems of its era. Understanding *why* languages exist—not just *how* to use them—makes you a better programmer, architect, and computer scientist.
...
- **Programming Language** — A structured system of communication for expressing computation
- **Computation** — Determination of new information from existing information by formal, mechanical means
- **Paradigm** — A fundamental style of programming (imperative, functional, object-oriented, logic)
...
- [Bret Victor: "The Future of Programming" (2013)](https://www.youtube.com/watch?v=8pTEmbeENF4) — A 1973-styled talk about ideas we still haven't fully embraced
- [Alan Kay: "Programming and Scaling" (2013)](https://www.youtube.com/watch?v=YyIQKBzIuBY) — The inventor of OOP reflects on what we got wrong

### Further Reading

- [Ray Toal: "The Study of Programming Languages"](https://cs.lmu.edu/~ray/notes/plstudy/) — The comprehensive reference this lecture draws from
- [Stanford Encyclopedia: History of Programming Languages](https://plato.stanford.edu/entries/computing-history/)

### Historical Papers

- Church, A. (1936) — "An Unsolvable Problem of Elementary Number Theory" (Lambda Calculus)
- Backus, J. (1959) — "The Syntax and Semantics of the Proposed International Algebraic Language" (BNF)
