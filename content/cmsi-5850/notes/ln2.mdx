...
- **What is Logic?** — The study of reasoning and inference
- **Building Blocks** — Terms, predicates, operators, formulas, and truth values
- **The Landscape of Logic** — Classical logic and its alternatives
- **Inference and Proof** — Natural deduction as mechanical reasoning
- **Formal Systems** — Soundness, completeness, and the Curry-Howard correspondence

---

## Introduction: What is Logic?

> "But in fact, they don't even know what thinking is. Because thinking is not actually logic. That was the mistake that the Greeks made." —Alan Kay
...
**Logic** is the study of **reasoning**. It deals primarily with **inference**—determining whether conclusions follow from premises—but also encompasses truth, validity, proof, and argumentation.
...
> **💡 Key Insight:** Logic gives us the machinery to make these distinctions precise. By the end of this lecture, you'll understand how to *mechanically verify* whether an argument is valid.

---

## Building Blocks of Logic

Let's build logic from first principles. Every logical statement is composed of four fundamental components: **terms**, **predicates**, **operators**, and **formulas**—which together evaluate to **truth values**.

---

### Terms — The Objects of Discourse

<DefinitionBox term="Term">

A **term** is the "noun" of logic—it refers to an object in our domain of discourse. Terms can be **constants** (specific objects) or **variables** (placeholders for any object).
...
Terms by themselves don't have truth values—they're just *things*. To make claims about things, we need predicates.

---

### Predicates — Testing Properties and Relations
...
> **📌 Key Point:** Predicates are where logic meets the world. They encode our knowledge about objects and their relationships.

---

### Operators (Connectives) — Combining Truth Values
...
> **🤔 Why is $F \supset T$ true?** This often confuses newcomers. Material implication $A \supset B$ only claims "whenever A is true, B is also true." If A is false, the implication makes no claim—it's *vacuously* true.

> **💡 Preview:** These five operators form the core of propositional logic, but many more exist! Modal logics add operators for necessity, possibility, obligation, knowledge, time, and more. We'll explore these in Section 4.
...
> **📌 Key Insight:** There isn't only one way to express logical ideas. Different formalisms have different strengths. The "best" choice depends on your purpose—clarity, efficiency, or proof automation.

---

### Formulas — Putting It All Together
...
- $\iota x. GodOfTheSea(x)$ — "The god of the sea" (i.e., Poseidon)
- $\iota x. King(england, x)$ — "The king of England"

> **⚠️ Caution:** The description operator can fail if no object satisfies the predicate (existence failure) or if multiple objects do (uniqueness failure). "The present king of France" is a famous example of existence failure.
...
2. $\forall x. Human(x) \supset Mortal(x)$ — "All humans are mortal"
3. $Mortal(socrates)$ — "Socrates is mortal"
...
> **🤔 Notice:** "Everyone loves someone" and "Someone is loved by everyone" have *different* logical forms! The order of quantifiers matters enormously.
...
We've assembled the components of what's called **classical logic**—but it's important to understand that this is just *one* logical system among many.

> ⚠️ **Important:** Axioms are not universally agreed upon. Different logical traditions have different foundational assumptions. What we call "classical logic" reflects one historically dominant choice—not the only possible one.
...
> **📌 Key Point:** Each of these features can be questioned, leading to alternative logics. Rejecting bivalence gives us multi-valued logics; rejecting excluded middle gives us intuitionistic logic; relaxing non-contradiction gives us paraconsistent logic.

---

### Beyond Classical Logic — Alternative Systems and Their Operators
...
In classical logic, from $A \land \neg A$, you can derive *anything* (ex falso quodlibet). Paraconsistent logics allow local contradictions without global collapse—useful for reasoning about inconsistent databases or paradoxes.
...
**Operator:** $\lvert A \rvert$ — the truth value of A, ranging from 0.0 to 1.0
...
**Key relationships:**
- $\Box A \equiv \neg \Diamond \neg A$ — "Necessarily A" = "Not possibly not-A"
- $\Diamond A \equiv \neg \Box \neg A$ — "Possibly A" = "Not necessarily not-A"

**Example:** "It's necessary that 2+2=4" → $\Box(2+2=4)$
...
**Key relationships:**
- $\mathscr{O} A \equiv \neg \mathscr{P} \neg A$ — "A is obligatory" = "Not-A is not permitted"
- $\mathscr{P} A \equiv \neg \mathscr{O} \neg A$ — "A is permitted" = "Not-A is not obligatory"

**Example:** "You must not steal" → $\mathscr{O} \neg Steal$
...
- $\mathscr{K}_x A \supset A$ — If x knows A, then A is true (knowledge is factive)
- $\mathscr{K}_x A \supset \mathscr{K}_x \mathscr{K}_x A$ — If x knows A, x knows that x knows A

**Example:** "Alice knows that Bob is home" → $\mathscr{K}_{alice} Home(bob)$
...
**Example:** "Romeo believes Juliet is dead" → $\mathscr{B}_{romeo} Dead(juliet)$
...
> **🤔 Philosophical Tangent: Is deduction "learning" anything new?**
>
> This question has puzzled philosophers for centuries. In one sense, deductive conclusions are already "contained" in the premises—we're just making explicit what was implicit.
>
...
**Introduction (∧I):** If you have A and you have B, you can conclude A ∧ B.

<DisplayMath formula="\frac{A \quad B}{A \land B} \text{ (∧I)}" />

**Elimination (∧E):** From A ∧ B, you can conclude A (or B).

<DisplayMath formula="\frac{A \land B}{A} \text{ (∧E₁)} \quad \frac{A \land B}{B} \text{ (∧E₂)}" />
...
**Introduction (∨I):** From A, you can conclude A ∨ B (for any B).

<DisplayMath formula="\frac{A}{A \lor B} \text{ (∨I₁)} \quad \frac{B}{A \lor B} \text{ (∨I₂)}" />

**Elimination (∨E):** From A ∨ B, if you can derive C from A and C from B, you can conclude C.

<DisplayMath formula="\frac{A \lor B \quad [A] \vdots C \quad [B] \vdots C}{C} \text{ (∨E)}" />
...
**Introduction (⊃I):** Assume A, derive B, then conclude A ⊃ B (discharging the assumption).

<DisplayMath formula="\frac{[A] \vdots B}{A \supset B} \text{ (⊃I)}" />

**Elimination (⊃E) — Modus Ponens:** From A ⊃ B and A, conclude B.

<DisplayMath formula="\frac{A \supset B \quad A}{B} \text{ (⊃E)}" />
...
**Introduction (¬I):** Assume A, derive a contradiction (⊥), then conclude ¬A.

<DisplayMath formula="\frac{[A] \vdots \bot}{\neg A} \text{ (¬I)}" />

**Elimination (¬E):** From A and ¬A, derive contradiction (⊥).
...
> **📌 Note:** This rule is *rejected* in intuitionistic logic! There, $\neg \neg P$ does not imply $P$.
...
<DisplayMath formula="\frac{\frac{[A \land B]^1}{B} \text{ (∧E₂)} \quad \frac{[A \land B]^1}{A} \text{ (∧E₁)}}{B \land A} \text{ (∧I)}" />

Then we discharge assumption 1 to complete the proof:

<DisplayMath formula="\frac{\frac{\frac{[A \land B]^1}{B} \text{ (∧E₂)} \quad \frac{[A \land B]^1}{A} \text{ (∧E₁)}}{B \land A} \text{ (∧I)}}{(A \land B) \supset (B \land A)} \text{ (⊃I)}^1" />

> **💡 Reading the tree:** Start at the leaves (top)—these are assumptions. Each horizontal bar applies a rule, combining what's above into what's below. The superscript $^1$ shows which assumption gets "discharged" when we apply ⊃I.
...
**Comparison to valid rules:** This superficially resembles **Modus Ponens** (⊃E), but Modus Ponens requires the *antecedent* ($A$), not the *consequent* ($B$). There is no valid rule that lets you "work backwards" from $B$ to $A$.
...
**Comparison to valid rules:** No elimination rule for implication allows us to derive anything about $B$ from $\neg A$. The implication $A \supset B$ only tells us what happens *when A is true*—it says nothing about what happens when A is false.
...
**Comparison to valid rules:** This violates the fundamental nature of proof. In natural deduction, to conclude $A$, we must either assume it or derive it from other facts—not merely note the absence of a proof of $\neg A$. This fallacy would make *every unproven statement* true!

> **💡 Key Insight:** Each fallacy corresponds to a *missing* inference rule. If you can't construct a valid derivation tree for a conclusion, the argument is suspect. The mechanical nature of natural deduction protects us from these errors.
...
This is a *syntactic* notion—it depends only on the rules of the system, not on meaning.
...
This is a *semantic* notion—it depends on meaning and truth, not on derivation rules.
...
| :--- | :--- | :--- | :--- |
| Sound | ✓ Yes | ✓ Yes | ✓ Yes |
| Complete | ✓ Yes | ✓ Yes (Gödel 1930) | ✗ No (Gödel 1931) |
| Decidable | ✓ Yes | ✗ No (Church-Turing 1936) | ✗ No |
...
The proof connects logic to computation through the **halting problem**: if we could decide theoremhood, we could decide whether arbitrary programs terminate—but Turing proved this is impossible.

> **⚠️ Important distinction:** Undecidability doesn't mean we can't prove *any* theorems. It means there's no *universal decision procedure*. For any specific formula, we might find a proof—but no algorithm can guarantee to find one (or determine none exists) for every formula.

**Practical implication:** This is why type-checkers for sufficiently powerful languages can "hang"—they're trying to solve an undecidable problem!

### Gödel's Incompleteness Theorems

In 1931, Kurt Gödel shattered the dream of a complete mathematical foundation with two devastating theorems:
...
You cannot have both consistency and completeness—this is a fundamental limit of formal reasoning.

> **📌 Why this matters:** These results don't mean mathematics or logic is "broken." They reveal deep truths about the nature of formal systems. In programming languages, they explain why some type systems are intentionally limited (to remain decidable) while others embrace undecidability for greater expressiveness.

---

### Why This Matters for Programming Languages

Logic isn't just abstract mathematics—it's the foundation of programming language theory.
...
**Example:** A function of type `A → B` is a "proof" that if you have an A, you can produce a B.

```haskell
-- The function is a proof of: A ⊃ (B ⊃ A)
const :: a -> b -> a
const x y = x
```

This correspondence means:
- **Type systems are logics** — A well-typed program is a valid proof
- **Proof assistants are programming languages** — Coq, Agda, Lean
- **Verified software** — Prove properties, get correct programs

> **💡 Course Connection:** This is why we study logic in a programming languages course! The tools for reasoning about programs come directly from formal logic.
...
- **Classical logic** is one choice among many—modal, temporal, deontic, and other logics extend it with specialized operators
...
- **The Curry-Howard correspondence** reveals that proofs are programs—the foundation of type theory
...
- **Logic** — The study of reasoning and inference
- **Term** — An object in the domain of discourse
- **Predicate** — A function from terms to truth values
- **Operator** — A connective that combines formulas
- **Formula** — A well-formed expression that can be evaluated for truth
- **Valid** — True under all interpretations (tautology)
- **Sound** — Every theorem is true
- **Complete** — Every true formula is a theorem

**The Core Inference Rules:**

| Operator | Introduction | Elimination |
| :--- | :--- | :--- |
| $\land$ | From A and B, derive A ∧ B | From A ∧ B, derive A (or B) |
| $\lor$ | From A, derive A ∨ B | Proof by cases |
| $\supset$ | Assume A, derive B, conclude A ⊃ B | Modus Ponens |
| $\neg$ | Assume A, derive ⊥, conclude ¬A | From A and ¬A, derive ⊥ |
...
- [Computerphile: "Gödel's Incompleteness Theorem" (2017)](https://www.youtube.com/watch?v=O4ndIDcDSGc) — Accessible explanation of why completeness is impossible
- [PBS Infinite Series: "Proving Brouwer's Fixed Point Theorem" (2017)](https://www.youtube.com/watch?v=csInNn6pfT4) — Constructive vs classical logic in action

### Further Reading

- [Ray Toal: Logic Notes](https://cs.lmu.edu/~ray/notes/logic/) — The comprehensive reference this lecture draws from
- [Stanford Encyclopedia: Classical Logic](https://plato.stanford.edu/entries/logic-classical/) — Philosophical foundations
- [Stanford Encyclopedia: Intuitionistic Logic](https://plato.stanford.edu/entries/logic-intuitionistic/) — Alternative to classical
- [Stanford Encyclopedia: Modal Logic](https://plato.stanford.edu/entries/logic-modal/) — Necessity and possibility
- [Propositions as Types (Philip Wadler)](https://homepages.inf.ed.ac.uk/wadler/papers/propositions-as-types/propositions-as-types.pdf) — Accessible introduction to Curry-Howard

### Historical Papers

- Frege, G. (1879) — "Begriffsschrift" (Concept-Script) — The birth of modern logic
- Russell, B. (1903) — "Principles of Mathematics" — Where type theory began
- Gödel, K. (1931) — "On Formally Undecidable Propositions" — The incompleteness theorems
- Gentzen, G. (1935) — "Investigations into Logical Deduction" — Natural deduction introduced
- Howard, W. (1969) — "The Formulae-as-Types Notion of Construction" — Curry-Howard correspondence

### Tools

- [Natural Deduction Proof Editor](https://proofs.openlogicproject.org/) — Practice building proofs
- [The Coq Proof Assistant](https://coq.inria.fr/) — Write verified programs
