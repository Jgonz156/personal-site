...
We also connected powersets and Cartesian products to type theory: sets inhabit powersets, tuples inhabit Cartesian products — set operations define the types that collections inhabit.
...
- **The Grand Operator Recap** — Everything we've built so far
- **Boolean Operators Are Special** — Relations test relationships
- **The Existential Quantifier** — "There exists an x such that..."
- **The Universal Quantifier** — "For all x..."
- **Quantifier Duality** — Proving and disproving quantified statements
- **Relational Properties** — Reflexivity, symmetry, antisymmetry, transitivity, totality
- **Equivalence Relations** — What makes things "equal"
- **Partial and Total Orders** — What makes things "orderable"
- **Relations in Python** — Dunder methods, sorting contracts, and hashability
...
That's a lot of operators! With these tools alone, all of the mathematics we've discovered so far can be encoded — with the asterisk that the knowledge we've acquired is still debated, and more is being discovered every day.

But now it's time to explore the tools we already have at a higher level. We've already seen **algebraic properties** like commutativity, associativity, and distributivity that describe how operators interact with their inputs, outputs, and each other. Today we discover another set of properties — ones that apply to a specific *kind* of operator.
...
Every operator that returns a **Bool** answers a yes-or-no question. It doesn't produce a new number, set, or tuple — it tests whether a *relationship* holds between its inputs.

Think about it:
- **Have you ever decided whether one program is better than another?** You became an operator that took two programs as input and returned yes or no — confirming the existence of a relationship between those programs.
...
These operators don't just compute — they **sort the world** into things that have some property and things that don't.
...
It tests whether a relationship holds between its inputs. If $R(a, b) = \top$, we say $a$ is **related to** $b$ under $R$.

</DefinitionBox>

The set of all pairs where $R$ returns true can be collected into a set:

<DisplayMath formula="R = \{(a, b) \in A \times A \mid R(a, b) = \top\}" />

That's a **set of tuples** — our two collection types working together once again! A relation is formally a subset of the Cartesian product.

> **📌 Key Insight:** Only operators that return Bool can be interpreted as testing relationships. Addition returns a number — it's not testing anything. Union returns a set — also not testing. But $\leq$ returns Bool — it's testing whether one number is at most another. If it returns a Bool, it's a relation.
...
The first is the **existential quantifier** — a way to make formal statements about the *existence* of something satisfying a condition.

<DefinitionBox term="Existential Quantifier (∃)">
...
| $\exists x \in \mathbb{Z},\; x^2 = -1$ | False | No integer squares to $-1$ |

### Proving Existential Statements

To show an existential statement is **true**, provide a **constructive proof** — exhibit a specific witness $x$ that satisfies $P(x)$. This is often the "easiest" direction: just find one example!

To show one is false... we'll need the universal quantifier first. Hold that thought.

### Python Connection

Python's `any()` is the existential quantifier:

```python
any(x > 3 for x in range(10))    # True — there exists an x > 3
any(x < 0 for x in range(10))    # False — no x < 0 in range(10)
any(x**2 == 25 for x in range(10))  # True — x = 5 works
```
...
The second upgrade is the **universal quantifier** — a way to make statements about *all* elements satisfying a condition.

<DefinitionBox term="Universal Quantifier (∀)">
...
To show a universal statement is **true**, you must demonstrate that it's **impossible** for any $x$ not to satisfy the property. This is generally harder than existential — you can't just find one example; you need to cover *all* cases.

To show a universal statement is **false**: find just **one counterexample**. That's all it takes — a single $x$ where $P(x)$ fails.

### Python Connection

Python's `all()` is the universal quantifier:

```python
all(x >= 0 for x in range(10))   # True — all x >= 0
all(x > 5 for x in range(10))    # False — x = 0 is a counterexample
all(x + 1 > x for x in range(100))  # True — always increases
```
...
This duality has a formal name — it's **De Morgan's Laws extended to quantifiers**:
...
Because saying "nothing exists with property P" requires checking *everything* — that's a universal claim. And saying "not everything has property P" only requires finding *one failure* — that's an existential claim. Negation swaps the difficulty, which swaps the quantifier.
...
Now we have the quantifiers we need. Let's use them to formally define **meta-properties** of relations — properties that describe the *nature* of how a relation behaves.
...
> **🤔 But wait...** Can a relation be *neither* reflexive *nor* irreflexive? Yes! Consider conjunction ($\land$) as a relation on booleans: $\top \land \top = \top$ (reflexive for True), but $\bot \land \bot = \bot$ (irreflexive for False). It's reflexive for some elements and irreflexive for others — so it's neither fully reflexive nor fully irreflexive.
...
| $=$ | Yes | Trivially — equality only goes both ways for identical items |
| "Has the same birthday as" | No | Two *different* people can share a birthday |

> **💡 Key Distinction:** Antisymmetric is NOT the opposite of symmetric! A relation can be *both* symmetric and antisymmetric (like $=$). The key difference: symmetric says "both ways is normal," antisymmetric says "both ways forces equality." A relation like "has the same birthday as" is symmetric but NOT antisymmetric (different people can share birthdays).
...
Intuition: the relation always has an opinion. There are no "incomparable" pairs — for any two elements, at least one direction holds.

| Relation | Total? | Why |
| :--- | :--- | :--- |
| $\leq$ on numbers | Yes | Any two numbers can be compared |
| $\geq$ on numbers | Yes | Any two numbers can be compared |
| $\subseteq$ on sets | **No** | $\{1,2\}$ and $\{2,3\}$ — neither is a subset of the other! |
...
> **💡 Side Note:** Totality implies reflexivity (just set $x = y$ in the formula). So a total relation is automatically reflexive.

---

## Why Do We Care?

Pause and reflect. These meta-properties tell us the **nature** of a relation — its "feel" or "truth."

Remember from LN6: associativity + commutativity together define what it means to be "addition-like." The operation isn't about numbers — it's about those properties! The same insight applies here: **combinations of relational properties define fundamental mathematical concepts**.
...
Being "equal" was never about what you were in particular — only that when you compare yourself to another, the comparison is reflexive, symmetric, and transitive!
...
| :--- | :---: | :---: | :---: | :---: |
| $=$ (equality) | ✓ | ✓ | ✓ | **Yes** |
| Logical equivalence ($\equiv$) | ✓ | ✓ | ✓ | **Yes** |
| Congruence mod $n$ | ✓ | ✓ | ✓ | **Yes** |
| "Has the same birthday as" | ✓ | ✓ | ✓ | **Yes** |
| "Is the same color as" | ✓ | ✓ | ✓ | **Yes** |
| $\leq$ | ✓ | ✗ | ✓ | No (not symmetric) |
| $\lt$ | ✗ | ✗ | ✓ | No (not reflexive) |
...
- **Symmetry**: if A is equivalent to B, then B is equivalent to A. Equivalence is mutual — it doesn't privilege one direction.
- **Transitivity**: equivalence chains. If A is equivalent to B, and B is equivalent to C, then A is equivalent to C.

All three together: an equivalence relation **partitions** a set into groups of "equivalent" items. Every element belongs to exactly one group. These groups are called **equivalence classes** — a concept we'll explore in future lectures.
...
- **Antisymmetry**: if $a \leq b$ and $b \leq a$, they must be equal. No distinct ties — two-way ordering forces identity.
- **Transitivity**: ordering chains. If $a \leq b$ and $b \leq c$, then $a \leq c$.

The word "partial" means: some pairs might be **incomparable**. With $\subseteq$ on sets, the sets $\{1,2\}$ and $\{2,3\}$ can't be compared — neither is a subset of the other. There's no "gap" to fill; they simply exist side by side without an ordering relationship.
...
**Not a total order:** $\subseteq$ on sets — it's a partial order but not total (some sets are incomparable).

This is why it's called "partial" — a partial order allows gaps where pairs can't be compared. A total order fills in all the gaps: every pair must be comparable.
...
The difference between partial and total orders is **totality** — whether every pair must be comparable.

> **🌟 Big Picture:** Just like associativity + commutativity make "addition," reflexivity + symmetry + transitivity make "equivalence," and reflexivity + antisymmetry + transitivity make "partial ordering." These aren't definitions we invented arbitrarily — they capture the essential *feel* of what it means for things to be equal or orderable.

---

## Relations in Python

Let's bring this back to code. Understanding relational properties makes you a better Python programmer — because Python's built-in functions **require** these properties as contracts.
...
Our `__eq__` is reflexive (same GPA = same GPA), symmetric (if A's GPA equals B's, then B's equals A's), and transitive — it's a valid equivalence relation!

And `__le__` based on GPA is reflexive, antisymmetric, and transitive — a valid partial order. Since every two GPAs (as numbers) are comparable, it's actually a total order.
...
If you override `__eq__` without defining `__hash__`, Python automatically makes your class **unhashable** — it's protecting the equivalence relation contract! Python knows that a broken equality-hash relationship would corrupt sets and dictionaries.
...
# Sets may now contain "duplicate" equal items — undefined behavior!
```

The `__eq__` says p1 and p2 are equivalent, but `__hash__` disagrees. This violates the contract, and sets can now contain what should be "the same" item multiple times — breaking the fundamental guarantee that sets have no duplicates.

> **📌 Key Insight:** Python's `sorted()` requires a total order. Python's `set` and `dict` require a valid equivalence relation (via `__eq__`) consistent with hashing. These aren't arbitrary API requirements — they're formal relational properties baked into the language. Understanding the math makes the Python contracts obvious.
...
- **Relation** — An operation $R : (A, A) \to$ Bool that tests a relationship
- **Existential Quantifier** ($\exists$) — $\exists x \in A,\; P(x)$: "there exists an $x$ such that..."
- **Universal Quantifier** ($\forall$) — $\forall x \in A,\; P(x)$: "for all $x$..."
- **Reflexive** — $\forall x,\; R(x, x)$
- **Irreflexive** — $\forall x,\; \lnot R(x, x)$
- **Symmetric** — $R(x, y) \to R(y, x)$
- **Asymmetric** — $R(x, y) \to \lnot R(y, x)$
- **Antisymmetric** — $(R(x, y) \land R(y, x)) \to x = y$
- **Transitive** — $(R(x, y) \land R(y, z)) \to R(x, z)$
- **Total** — $\forall x, y,\; R(x, y) \lor R(y, x)$
...
- [Trefor Bazett: "Relations and Their Properties"](https://www.youtube.com/watch?v=GQSH2aN6JfA) — Clear walkthrough of reflexivity, symmetry, transitivity
- [Computerphile: "Equivalence Relations"](https://www.youtube.com/watch?v=2S4nn7S8Hk4) — Accessible introduction with examples

### Further Reading

- [Wikipedia: Binary Relation](https://en.wikipedia.org/wiki/Binary_relation) — Comprehensive treatment of relation properties
- [Wikipedia: Equivalence Relation](https://en.wikipedia.org/wiki/Equivalence_relation) — Reflexive + symmetric + transitive
- [Wikipedia: Partial Order](https://en.wikipedia.org/wiki/Partially_ordered_set) — Reflexive + antisymmetric + transitive
- [Wikipedia: Total Order](https://en.wikipedia.org/wiki/Total_order) — When every pair is comparable

### Historical Context

- **Augustus De Morgan** (1806-1871) — Quantifier duality extends his famous laws to quantifiers
- **Ernst Schröder** (1841-1902) — Pioneered the algebra of relations
- **Gottlob Frege** (1848-1925) — Formalized quantifiers in modern predicate logic
...
- `sorted()` requires a total order — violating transitivity causes undefined sorting behavior
