...
Last time we explored the **Boolean Type** through George Boole's historical discovery. We expanded on **reducibility** — how different systems can solve the same problem — and used that insight to visualize logical operations with **Venn Diagrams**.

---

## Today's Agenda

- **Operator Syntax** — Types tell us what operators mean
- **Operator Notation** — Many ways to write the same operation  
- **The Big Reveal** — Addition isn't about numbers!
...
This is called **overloading** — the `+` symbol means different things based on context!
...
- **Infix** ($a + b$) — Familiar, reads naturally
- **Prefix** ($+\ a\ b$) — Chains easily: $+\ a\ b\ c\ d$ means $a + b + c + d$
- **Postfix** ($a\ b\ +$) — No parentheses needed, used in calculators
...
Fractions use **vertical** layout. Exponents use **superscript**. Integrals wrap around their argument. These aren't infix, prefix, or postfix — they're custom notations designed for clarity.
...
> **💡 Key Insight:** Syntax is flexible! The **type** tells us what an operator does. The **notation** is just how we choose to write it. Different fields use different notations because they clarify different things.
...
<NumberLine
  ...
  caption="Addition without numbers — just movement!"
/>
...
<NumberLine
  ...
  showNumbers={false}
  hopLabels={["hop hop", "hop hop hop"]}
  caption="Same operation, no numbers — still works!"
/>
...
The numbers are just labels for positions — the actual operation is **combining movements**.

> **💡 Key Insight:** Addition is about **movement/direction**, not values. The number line taught us this as children, but we forgot it when we learned algebra!
...
Again, the result "5" is the length of the combined chain — no numbers needed!

> **💡 Key Insight:** Addition is about **combining/concatenating** — linking things together. No arithmetic required!
...
1. **Number line** — combining movements
2. **Graphs** — combining chains
3. **Strings** — combining text
...
<NumberLine
  ...
  showNumbers={true}
  hopLabels={["+4", "+1"]}
  caption="4 + 1: hop 4, then hop 1 — same destination!"
/>

Both paths land at position 5! The number line is **commutative**. ✓
...
Whether we connect "1 node to 4 nodes" or "4 nodes to 1 node", we get the same 5-node chain. ✓
...
**Wait!** These are NOT the same! String concatenation is **NOT commutative**! ✗

> 🚨 **Important:** We found our first difference! Keep this in mind...
...
**Numbers:** `(1 + 2) + 3 = 3 + 3 = 6` and `1 + (2 + 3) = 1 + 5 = 6` ✓
...
Both arrive at 7! Whether we compute `(1+4)+2 = 5+2 = 7` or `1+(4+2) = 1+6 = 7`, the destination is the same. Grouping doesn't matter. ✓

**Graph Chains:** Whether we connect (A-B) then C, or A then (B-C), we get the same chain. ✓
...
Both give `"abc"`! String concatenation IS **associative**. ✓
...
| :--- | :--- | :--- | :--- |
| Number addition | ✓ | ✓ | **Yes** |
| Number line hops | ✓ | ✓ | **Yes** |
| Graph chain building | ✓ | ✓ | **Yes** |
...
Yes! Multiplication satisfies both properties — it's structurally "additive"!
...
> **🤯 Mind-Blown Moment:** Multiplication (×) is mathematically "addition" even though it uses a different symbol. String concatenation (+) is NOT mathematically "addition" even though it uses the + symbol!
...
> **📚 Formal Term:** String concatenation forms a **monoid** (associative with identity), but not an **abelian group** (which requires commutativity). Addition, multiplication, AND, and OR all form abelian groups over their domains!
...
An operation `⊕` has an **identity element** `e` if combining anything with `e` leaves it unchanged:

`a ⊕ e = a` and `e ⊕ a = a`

</DefinitionBox>

Every "addition-like" operation has an identity — something that "does nothing":

| System | Operation | Identity Element | Example |
| :--- | :--- | :--- | :--- |
| Natural numbers | Addition | `0` | `5 + 0 = 5` |
| Natural numbers | Multiplication | `1` | `5 × 1 = 5` |
| Number line | Hops | 0-hop (stay in place) | `position + 0 = position` |
| Graphs | Chain building | Empty chain | `chain ⊕ ∅ = chain` |
| Strings | Concatenation | `""` (empty string) | `"hello" + "" = "hello"` |
| Booleans | OR (∨) | `False` | `A ∨ False = A` |
| Booleans | AND (∧) | `True` | `A ∧ True = A` |
...
> **💡 Key Insight:** Even string concatenation has an identity (the empty string). Having an identity doesn't make something "addition" — you also need commutativity!
...
```
+ : (Natural, Natural) -> Natural   ✓ Closed!
∧ : (Boolean, Boolean) -> Boolean   ✓ Closed!
+ : (String, String) -> String      ✓ Closed!
```

The input types match the output type — the operation never "escapes" to a different type.
...
What's `3 - 5`? It's `-2`, which is NOT a natural number! Subtraction on naturals is **NOT closed** — the output type doesn't match the input type. We'd have to write:

```
- : (Natural, Natural) -> Integer   ✗ Not closed!
```

| Type | Operation | Type Signature | Closed? |
| :--- | :--- | :--- | :--- |
| Natural | Addition | `(Natural, Natural) -> Natural` | ✓ |
| Natural | Subtraction | `(Natural, Natural) -> Integer` | **✗** |
...
If an operation isn't closed, you can "escape" the type! Subtraction on naturals can produce integers, which aren't naturals. This is why mathematicians invented integers — to make subtraction closed!

> **📚 Set Theory Note:** In set theory, this is called being "closed over a set" — same idea, different vocabulary. When you take a course on set theory, you'll see closure defined in terms of sets rather than types, but the concept is identical.
...
| Natural Numbers | Multiplication | `×` | Yes | Yes | 1 | Yes | **Yes!** |
| Number Line | Hops | → | Yes | Yes | 0-hop | Yes | **Yes** |
| Graphs | Chain Building | ⊕ | Yes | Yes | Empty | Yes | **Yes** |
| Strings | Concatenation | `+` | **No** | Yes | "" | Yes | **No!** |
| Booleans | Disjunction (OR) | `∨` | Yes | Yes | False | Yes | **Yes!** |
| Booleans | Conjunction (AND) | `∧` | Yes | Yes | True | Yes | **Yes!** |

**The Surprising Truth:**
- Multiplication (×) IS mathematically "addition" — despite its different name and symbol
- String concatenation (+) is NOT mathematically "addition" — despite using the + symbol
- AND and OR are both mathematically "addition" — they're just addition over different domains!
...
> **🔮 Looking Ahead:** These properties will return when you study:
> - **Linear Algebra** — Vector addition forms an abelian group
> - **Cryptography** — Security relies on group theory
> - **Programming Language Theory** — Type systems use algebraic structures
> - **Category Theory** — The "algebra of composition"
...
- **Identity Element** — An element `e` that leaves others unchanged: `a ⊕ e = a`
- **Closure** — Output type matches input types: `⊕ : (A, A) → A`
...
The symbol doesn't define the operation — the **properties** do:
- Multiplication uses `×` but IS mathematically "addition"
...
- [3Blue1Brown: "Group Theory, Pair of Pants" (2023)](https://www.youtube.com/watch?v=X63MWZIN3gM) — Visual introduction to algebraic structures
- [Numberphile: "Abstract Algebra" (2019)](https://www.youtube.com/watch?v=IP7nW_hKB7I) — Accessible introduction to the field

### Further Reading

- [Wikipedia: Commutative Property](https://en.wikipedia.org/wiki/Commutative_property) — Overview with examples
- [Wikipedia: Associative Property](https://en.wikipedia.org/wiki/Associative_property) — Overview with examples
- [Wikipedia: Monoid](https://en.wikipedia.org/wiki/Monoid) — The structure string concatenation forms
- [Wikipedia: Abelian Group](https://en.wikipedia.org/wiki/Abelian_group) — The structure "addition" forms

### Historical Context

- **Évariste Galois** (1811-1832) — Founder of group theory, died in a duel at age 20
- **Niels Henrik Abel** (1802-1829) — "Abelian" groups are named after him
- **Emmy Noether** (1882-1935) — Revolutionized abstract algebra and its connection to physics

### Connection to Programming

- [Haskell Typeclassopedia](https://wiki.haskell.org/Typeclassopedia) — How algebraic structures appear in functional programming
