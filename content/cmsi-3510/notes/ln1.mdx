...
- **Syntax** — How programming languages *look*
- **Semantics** — What programming languages *mean*
- **Case Study** — Python vs. Rust: Making Change
...
Syntax is all about **structure**—the rules that govern how symbols can be arranged. Think of it like the grammar of a language, but stripped of all meaning.
...
> ⚠️ **WAIT!** Before we continue, it's important to note that we are NOT talking about the *meaning* of the sentence. We are only talking about the *structure*. Do your best to **ignore the meaning of the words!**

We can abstract away the meaning entirely:

```
"The cat likes to eat fish."  →  "Noun-Phrase Verb Infinitive Noun"
```
...
- Maybe even *"fan is a bob of the lakers"*? 🤷

We simply can't tell! And **that's the point**—syntax is about *structure*, not meaning.
...
An **expression** is a combination of values, variables, operators, and function calls that **evaluates to a single value**. Think of it like a math problem—there's a clear order of operations that reduces complexity step by step.

</DefinitionBox>

<DefinitionBox term="Statement">

A **statement** is a complete instruction that performs an action. In Python, statements generally do not evaluate to a value the way expressions do.
...
Statements control program flow—they don't "simplify" to anything:
...
> **📌 Key Insight:** Syntax gives us rules for how to validly organize **tokens** in a programming language. A *token* is the smallest unit of meaningful information—a single "piece" of the language.
...
While syntax is about *structure*, semantics is about *meaning*—the actual *stuff* we store in our language's symbols.
...
- `red` → stores information about the color red
- `car` → stores information about vehicles with four wheels and a motor
...
7. **EOF**: End of file reached → terminate program
...
> **💡 Observation:** Rust's larger syntax vocabulary allows it to store more *specific* semantic information directly in the code structure!

---

## Summary

- **Syntax** is the structure and grammar of code — the rules for how tokens can be arranged
- **Semantics** is the meaning and behavior of code — the actual information stored in those arrangements
...
- Different languages can express the *same semantics* with different syntax — the Making Change case study showed Python and Rust implementing identical logic with different syntactic vocabularies
...
- [Programming Language Pragmatics (Scott)](https://www.cs.rochester.edu/~scott/pragmatics/) — Deep dive into syntax, semantics, and language design
- [Crafting Interpreters (Nystrom)](https://craftinginterpreters.com/) — Free online book walking through tokenization, parsing, and evaluation
- [Python AST Documentation](https://docs.python.org/3/library/ast.html) — Explore Python's abstract syntax tree to see syntax structure in action

### Rust Systems Programming

- [Rust by Example](https://doc.rust-lang.org/rust-by-example/) — Learn Rust through annotated examples
- [The Rust Book: Getting Started](https://doc.rust-lang.org/book/ch01-00-getting-started.html) — Official introduction to the Rust toolchain
