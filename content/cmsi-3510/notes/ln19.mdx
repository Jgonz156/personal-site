...
{/* U-BEND INTERACTIVE: Full request-path stepper goes here.
    Shape: horseshoe/U with left tip at top-left, right tip at top-right, bend at bottom.
    Left tip  = process calls read(), begins wait  (user space)
    Left descent = syscall/trap crosses boundary
    Left side of bend = kernel dispatches to driver  (kernel space)
    Bottom of bend = driver programs hardware
    Fork point = process sleeps / device runs independently (U bifurcates into two paths)
    Right side of bend = interrupt arrives, deferred work runs  (kernel space)
    Right ascent = process wakes, crosses boundary back
    Right tip  = process receives bytes, continues  (user space)
    A horizontal line across the top of the U marks the user-space / kernel-space boundary.
    Color-coded phases: each section of the lecture highlights its current phase.
    Sub-variants of this diagram appear in each section below with the active phase emphasized. */}
...
{/* U-BEND VARIANT: Phase 1 — highlight the left descent crossing the user/kernel boundary line.
    The process step "call read()" is at the left tip (done).
    The syscall arrow crosses the horizontal boundary into kernel space.
    Everything below and to the right remains dimmed. */}
...
{/* U-BEND VARIANT: Phase 2 — highlight the left side of the bend (kernel space).
    The trap crossing is complete (shown dimmed/done). Now we are inside the kernel,
    and the dispatch path from syscall layer → VFS/device layer → driver is highlighted. */}
...
{/* U-BEND VARIANT: Phase 3 — highlight the bottom of the bend.
    Dispatch is done (dimmed). We are at the deepest point of privilege, where the driver
    directly interacts with hardware registers and device controllers.
    This is the furthest point from user space in the entire path. */}
...
{/* U-BEND VARIANT: Phase 4 — the U-bend bifurcates here.
    The single path splits into two parallel tracks diverging from the bottom of the bend:
    Left track: the process path — process is marked blocked/sleeping (greyed out, dormant)
    Right track: the device path — device is actively measuring (highlighted, active)
    Both tracks run independently. The visual should convey that the CPU is free
    and the device is operating on its own clock. The two tracks will merge at Phase 5.
    This bifurcation is the key visual innovation of the U-bend. */}
...
{/* U-BEND VARIANT: Phase 5 — the two forked tracks rejoin.
    The device track fires an interrupt (highlighted with a lightning bolt or signal).
    The interrupt handler runs briefly in interrupt context.
    Deferred work is scheduled. The process track transitions from blocked → ready.
    The two tracks merge back into a single path ascending toward user space.
    The right ascent crosses the user/kernel boundary line back into user space.
    The right tip shows the process receiving bytes and continuing.
    This variant should show the rejoining clearly — two lines becoming one again. */}
