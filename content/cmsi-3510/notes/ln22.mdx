...
{/* LAYER MODEL STATIC VISUAL: The TCP/IP layer stack from LN21, displayed vertically.
    Rows from bottom to top: Physical (folded into Link), Link, Network, Transport, Application.
    The Link and Network rows should appear filled-in / solid — these were covered in LN21.
    The Transport row should be highlighted, glowing, or outlined in an accent color
    with a label: "Today's lecture."
    The Application row should be dimmed or shown as a dashed outline —
    it sits above transport but is not the focus of this lecture.
    This visual should be compact (not a full-page diagram) and serve as a
    quick orientation: "you are here in the stack." */}
...
{/* PACKET VIEW VS CONVERSATION VIEW: Two synchronized panels.
    Left panel: raw packet motion across a network with multiple packet boxes,
    headers, possible reordering, and timing gaps.
    Right panel: one simple client/server exchange labeled request -> reply.
    The left side should feel mechanically busy. The right side should feel
    like what a programmer actually wants to think about.
    Animation: as several packets move on the left, the right side keeps the
    single higher-level intent visible: "client asks", "server answers." */}
...
{/* SOCKET ABSTRACTION VISUAL: Vertical layered diagram.
    Application at the top calls send/recv on a socket handle.
    Below that: socket object in the kernel.
    Below that: transport logic.
    Below that: packet path from LN21.
    The visual should emphasize compression: a tiny API at the top resting on
    a much wider and more complicated stack below.
    Label the socket as the "narrow waist" that the program actually sees. */}
...
{/* HOST MULTIPLEXING VISUAL: One machine with one NIC and one IP address.
    Inside the host are many sockets, each with a labeled local port.
    Incoming arrows arrive at the machine, then fan inward to different sockets.
    Outgoing arrows originate from many sockets and merge onto one interface.
    The visual should make the host feel like a switchboard operator:
    one external face, many internal conversations. */}
...
{/* WELL-KNOWN PORT ANIMATION:
    Layout: Two machines — a Client (left) and a Server (right).
    The server shows a vertical column of port slots. Port 80 is highlighted
    and labeled "listening" with a door icon or open-gate metaphor.
    The client shows its own column of port slots, mostly empty.

    Phase 1 — Discovery:
      Client wants to reach the HTTP server. It knows the well-known port: 80.
      An arrow reaches from the client toward port 80 on the server.
      The client's OS assigns an ephemeral port (e.g., 53014) automatically.
      Label appears: 4-tuple (192.168.1.10:53014 ↔ 203.0.113.10:80).

    Phase 2 — accept() creates a new socket:
      The server's port 80 "listening" socket remains in place (still glowing).
      A NEW socket appears on the server, labeled with the full 4-tuple.
      The arrow from the client now connects to this new socket, NOT directly
      to port 80's listener. Port 80 is free to accept again.

    Phase 3 — Second client arrives:
      A second client (different IP or different ephemeral port) sends a
      connection request to port 80.
      accept() fires again. Another new socket appears on the server with
      a DIFFERENT 4-tuple.
      Now the server has two active conversations plus the still-listening
      port 80 socket.

    Phase 4 — Many clients:
      Several more clients connect. The server accumulates many active sockets,
      each with a unique 4-tuple. Port 80's listener is always available.
      The visual should make it clear: the well-known port is the rendezvous
      point for discovery. The 4-tuple is the identity of the actual conversation.

    Key visual cues:
      - The listening socket stays visually distinct (a door, a gate, a beacon).
      - Each accepted socket is a separate object with its own 4-tuple label.
      - Ephemeral ports on the client side should look temporary / auto-assigned.
      - The server's port 80 never "fills up" — it just keeps spawning new sockets. */}
...
{/* UDP CONTRACT VISUAL: Same temperature-service example shown with a lightweight path.
    Small datagram blocks go from client to server and back.
    Guarantees panel shows only a few boxes checked: ports, checksums, boundaries.
    Unchecked boxes: reliability, ordering, retransmission, connection state.
    The diagram should feel fast and low-ceremony. */}
...
{/* UDP VS TCP CONTRACT COMPARISON: Same client/server example rendered twice.
    Left side (UDP): one datagram out, one datagram back, sparse state.
    Right side (TCP): handshake first, then stream data, acknowledgments,
    retransmission possibility, and receiver window / flow-control cues.
    The guarantee panel should compare the two as contracts, not as winners.
    Use phrasing like "minimal contract" versus "stateful contract." */}
...
{/* WAITING-MODEL TIMELINE: Three horizontal timelines for one process.
    Row 1: Blocking socket — process sleeps on one call until data arrives.
    Row 2: Nonblocking socket — process keeps running, repeatedly encountering
    "would block" until data becomes available.
    Row 3: Readiness model — process waits once on the kernel for any of many
    sockets to become ready, then services the ready ones.
    The visual should tie back to scheduling and wakeup rather than feeling like
    a pure networking artifact. */}
...
{/* ABSTRACTION COMPRESSION VISUAL: Three stacked views of the same conversation.
    Top: Application view — "client asks", "server replies".
    Middle: Socket/transport view — endpoints, ports, connection state, readiness.
    Bottom: Packet/path view — headers, packets, hops, NICs, DMA, interrupts.
    The middle layer should look like a compression of the bottom layer and a
    support beam for the top layer. */}
