...
- **Pass-by-Value vs Pass-by-Reference** — When to move vs borrow
- **Closures** — Anonymous functions that capture their environment
- **Methods and Associated Functions** — Organizing code with `impl` blocks
- **Dynamic Dispatch** — Runtime polymorphism with `dyn Trait`
...
- **Structs** — Defining custom types with ownership semantics
- **Enums** — Tagged unions that carry data
- **Traits** — Rust's approach to polymorphism (no inheritance!)
- **Standard Library Types** — Smart pointers and common patterns
...
A **struct** (structure) is a custom data type that groups related values under a single name. Each piece of data is called a **field**. Structs own their data—when a struct is dropped, all its fields are dropped too.
...
> **💡 Connection to LN3:** This is the ownership system at work! The struct is the owner, and when the owner is dropped, all owned values are deallocated.
...
> **⚠️ Watch out for moves!** The `..` syntax **moves** non-Copy fields. After this, `user1.username` is invalid (it was moved to `user2`), but `user1.active` and `user1.sign_in_count` are still valid (they implement `Copy`).
...
println!("{}", user1.sign_in_count); // ✓ OK: u64 is Copy
```
...
fn main() {
    ...
    // calculate_runway_length(altitude);  // ✗ ERROR: expected Meters, found Feet
}
```

> **💡 Key Insight:** The newtype pattern gives you **type safety at compile time** with **zero runtime overhead**. `Meters(100.0)` and `100.0_f64` have identical memory representations.
...
Structs with no fields—they're **zero-sized types (ZSTs)**:
...
This is a remarkably powerful concept: **information that exists purely in the type system, not in memory**. Traditional programming stores all information as bits in RAM—but marker structs let you encode constraints, states, and properties that the compiler tracks without any runtime cost.
...
But with markers, that information moves "out of band"—into the type system itself:
...
> **💡 Key Insight:** This is "zero-cost abstraction" taken to its logical extreme. The marker adds **zero bytes** to your struct, yet provides **compile-time guarantees** that would otherwise require runtime checks. Bugs that would be caught at runtime (or worse, in production) are now caught at compile time.
...
fn main() {
    ...
    readonly_file.read();        // ✓ OK
    // readonly_file.write(&[]);  // ✗ ERROR: no method `write` for FileHandle<ReadOnly>
    // readonly_file.delete();    // ✗ ERROR: no method `delete` for FileHandle<ReadOnly>
    
    admin_file.read();           // ✓ OK
    admin_file.write(&[]);       // ✓ OK
    admin_file.delete();         // ✓ OK
}
```

> **📌 OS Relevance:** This pattern mirrors how operating systems handle file permissions—but instead of checking permissions at runtime (and potentially failing), Rust checks them at compile time. Invalid operations simply don't compile.
...
┌────┬────┬────┬────┐            ┌────┬────┬────┬────┐
│ b  │ b  │ b  │ b  │ (u32)      │ a  │pad │pad │pad │
├────┼────┼────┼────┤            ├────┼────┼────┼────┤
│ a  │ c  │pad │pad │            │ b  │ b  │ b  │ b  │ (u32)
└────┴────┴────┴────┘            ├────┼────┼────┼────┤
Total: 8 bytes                   │ c  │pad │pad │pad │
                                 └────┴────┴────┴────┘
                                 Total: 12 bytes
```

> **📌 OS Relevance:** When writing device drivers or syscall interfaces, `#[repr(C)]` ensures your struct layout matches what the OS expects.
...
> **🤔 Why lifetimes?** The compiler needs to ensure the reference inside the struct doesn't outlive the data it points to. The `'a` annotation says "this struct cannot outlive the reference it contains."

```rust
// This won't compile:
fn create_excerpt() -> Excerpt<'static> {
    let text = String::from("temporary");
    Excerpt { content: &text }  // ✗ ERROR: `text` doesn't live long enough
}
```
...
An **enum** (enumeration) defines a type that can be one of several **variants**. Unlike C enums, Rust enums can carry data—each variant can have different types and amounts of associated data.
...
> **💡 Key Insight:** This is what makes Rust enums incredibly powerful. Each variant is like a mini-struct inside the enum. You can represent complex state machines, protocol messages, or AST nodes elegantly.
...
│   (1-8 bytes)│      (size of largest variant)      │
└──────────────┴─────────────────────────────────────┘
```

> **💡 Why only the largest?** This is fundamentally different from structs! A struct's fields all exist simultaneously—if you have `username`, `email`, and `active`, all three are stored in memory at once. But enum variants are **mutually exclusive**: a `Message` is *either* `Quit` *or* `Move` *or* `Write`—never more than one at a time. Since only one variant can exist at any moment, we only need enough space for the biggest possibility. The discriminant tells us which variant is currently "active."
...
> **📌 Key Point:** This is why `Option<Box<T>>` and `Option<&T>` can often have no extra size overhead compared to nullable pointers in C—Rust may use the null representation for `None`.

---

### Pattern Matching with Enums

The `match` expression is exhaustive—you must handle all variants:
...
In languages like C, Java, or Python, any reference can be null/None. This leads to the infamous **null pointer exception**—Tony Hoare, who invented null references, called it his "billion-dollar mistake."

```c
// C: Null pointer dereference - undefined behavior (crash, security vulnerability)
char* name = get_user_name(id);
printf("%s", name);  // What if name is NULL? 💥

// Java: NullPointerException at runtime
String name = getUserName(id);
System.out.println(name.length());  // What if name is null? 💥
```
...
// println!("{}", name.len());  // ✗ ERROR: Option<String> has no method `len`
...
> **💡 Key Insight:** With `Option`, you can't accidentally use a value that might not exist. The compiler forces you to handle the `None` case.
...
> **💡 Key Insight:** You can't ignore a `Result`. If you try to use it without handling the error case, the compiler warns you. Errors are part of the API contract.
...
| `unwrap_or_default()` | ✓ | ✓ | Get value or use `Default::default()` |
| `map(f)` | ✓ | ✓ | Transform the success value |
| `map_err(f)` | — | ✓ | Transform the error value |
| `and_then(f)` | ✓ | ✓ | Chain operations that might fail |
| `or_else(f)` | ✓ | ✓ | Try alternative if failed |
| `is_some()` / `is_ok()` | ✓ | ✓ | Check for success |
| `is_none()` / `is_err()` | ✓ | ✓ | Check for failure |
| `ok()` | — | ✓ | Convert `Result` to `Option` |
| `err()` | — | ✓ | Extract error as `Option` |
| `ok_or(e)` | ✓ | — | Convert `Option` to `Result` |
| `?` operator | ✓ | ✓ | Early return on failure |

> **📌 OS Relevance:** System calls can fail for many reasons—file not found, permission denied, resource exhausted. Rust's `Result` type makes these failures explicit, forcing you to handle them rather than silently ignoring return codes (a common source of bugs in C programs).
...
A **trait** defines shared behavior—a collection of methods that types can implement. Traits are Rust's primary mechanism for polymorphism, replacing inheritance found in languages like Java or C++.
...
```rust
// ✓ OK: We defined Summary
impl Summary for String { ... }

// ✓ OK: We defined Article  
impl Display for Article { ... }

// ✗ ERROR: Neither Display nor String is ours
impl Display for String { ... }
```

> **💡 Key Insight:** This is why the newtype pattern exists! Wrap the foreign type in your own struct, then implement traits on that.
...
> **📌 Key Point:** Traits provide **horizontal** code sharing (any type can implement any trait) rather than **vertical** inheritance hierarchies.
...
> **💡 Key Insight:** `derive` generates boilerplate implementations. For custom behavior, implement the trait manually.
...
> **📌 OS Relevance:** The `Drop` trait is essential for RAII (Resource Acquisition Is Initialization). File handles, mutexes, network sockets—anything that needs cleanup implements `Drop`.
...
> **📌 Note:** We'll cover these in depth in a future lecture on concurrency.
...
- **Struct** — Custom data type grouping related values; owns its fields
- **Enum** — Type with multiple variants, each potentially holding different data
- **Trait** — Collection of methods defining shared behavior
- **Newtype** — Single-field tuple struct for type safety
- **ZST** — Zero-sized type, takes no memory
...
- [The Rust Book: Structs](https://doc.rust-lang.org/book/ch05-00-structs.html) — Comprehensive struct guide
- [The Rust Book: Enums](https://doc.rust-lang.org/book/ch06-00-enums.html) — Enums and pattern matching
- [The Rust Book: Traits](https://doc.rust-lang.org/book/ch10-02-traits.html) — Trait fundamentals
- [Rust By Example: Custom Types](https://doc.rust-lang.org/rust-by-example/custom_types.html) — Interactive examples

### Advanced Topics

- [The Rustonomicon: Data Layout](https://doc.rust-lang.org/nomicon/data.html) — Memory layout details
- [The Rustonomicon: Repr](https://doc.rust-lang.org/nomicon/other-reprs.html) — `#[repr(...)]` attributes
- [Rust Reference: Type Layout](https://doc.rust-lang.org/reference/type-layout.html) — Official specification

### Standard Library Documentation

- [std::option::Option](https://doc.rust-lang.org/std/option/enum.Option.html) — Option enum
- [std::result::Result](https://doc.rust-lang.org/std/result/enum.Result.html) — Result enum
- [std::ops](https://doc.rust-lang.org/std/ops/) — Operator traits
- [std::convert](https://doc.rust-lang.org/std/convert/) — From/Into traits
