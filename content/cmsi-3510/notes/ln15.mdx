...
import { PageBitSacrifice } from "@/components/page-bit-sacrifice"
import { EightBitTranslationChain } from "@/components/eight-bit-translation-chain"
...
1. **The Universal Memory Dream** — Revisiting LN13's ideal and why we need virtual memory
2. **What Virtual Memory Is (and Is Not)** — Clearing up the biggest misconception
3. **Non-Contiguous Allocation** — Breaking free from contiguous placement
4. **Paging** — Dividing memory into fixed-size pages and frames
5. **Page Faults and Swapping** — What happens when a page is not in RAM
6. **The MMU in Action** — Hardware-driven address translation
7. **Page Tables** — The data structure that maps pages to frames
8. **An 8-Bit System** — Walking through a complete paging example end-to-end
...
An 8-bit instruction can reference 2<sup>8</sup> = 256 different addresses. That is the entire <MemoryTag type="logical">logical address space</MemoryTag> of this system — 256 lines, numbered 0 to 255. Every logical address the CPU can express fits in those 8 bits.
...
Now we cross into the <MemoryTag type="physical">physical</MemoryTag> side. Let us commit to the **3-bit page scenario** (8 pages of 32 lines each) and ask: how much physical memory do we need?

Each <MemoryTag type="physical">frame</MemoryTag> must be exactly 32 bytes — the same size as a page. If our physical memory is smaller than 32 bytes, we cannot even fit a single frame. If it is exactly 32 bytes, we get one frame. If it is 64 bytes, we get two. We will go with **64 bytes** and see what the page table looks like.
