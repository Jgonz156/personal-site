...
A **function** in Rust is a named, reusable block of code with explicitly typed parameters and an optional return type. Functions are compiled to static machine code—their types are fully resolved at compile time.
...
A **closure** is an anonymous function that can capture variables from its surrounding environment. Unlike regular functions, closures can "close over" their environment—accessing and even owning variables from the scope where they're defined.
...
1. **No return value needs to be stored** — no stack space for the result
2. **Code after the call is dead** — it can be eliminated
3. **Control flow doesn't continue** — simplifies analysis
...
1. **Monomorphization** — Generics and `impl Trait` generate specialized code at compile time
2. **Zero-cost closures** — Closures compile to structs; no heap allocation unless boxed
3. **Static dispatch by default** — Only pay for dynamic dispatch when you explicitly use `dyn`
4. **Pass small types by value** — Avoids indirection for types ≤16 bytes
5. **Pass large types by reference** — Avoids copying for larger structures
...
- [The Rust Book: Functions](https://doc.rust-lang.org/book/ch03-03-how-functions-work.html) — Official introduction to functions
- [The Rust Book: Closures](https://doc.rust-lang.org/book/ch13-01-closures.html) — Comprehensive closure guide
- [The Rust Book: Traits](https://doc.rust-lang.org/book/ch10-02-traits.html) — Understanding trait bounds
- [The Rust Reference: Never Type](https://doc.rust-lang.org/reference/types/never.html) — The `!` type specification
- [Rust By Example: Functions](https://doc.rust-lang.org/rust-by-example/fn.html) — Practical function examples
- [The Rustonomicon: Higher-Rank Trait Bounds](https://doc.rust-lang.org/nomicon/hrtb.html) — Advanced function signatures
