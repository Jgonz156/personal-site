...
The word "static" in SRAM does not mean "unchanging data" — it means the circuit is **statically stable**. Unlike its dynamic cousin, SRAM doesn't need to be constantly refreshed. The tradeoff? Six transistors per bit makes SRAM *expensive* and *physically large* relative to the data it stores. You cannot cheaply build terabytes of SRAM — you would need billions of transistors per byte.
...
The word "dynamic" refers to the fact that the stored charge is constantly *draining away*. The memory controller must periodically read each bit and write it back to refresh the capacitor — a process that takes time and bandwidth. If the refresh doesn't happen fast enough, the bit "decoheres" and the data is lost, even while the system is powered on.

Why use DRAM at all if it has this problem? Because 1 transistor + 1 capacitor is **dramatically** cheaper and smaller than 6 transistors. You can pack orders of magnitude more DRAM into the same silicon area. This makes DRAM suitable for **main memory** — those sticks of RAM you slot into a desktop motherboard. Modern DDR5 modules reach 4800–8400 MT/s (megatransfers per second) with on-die ECC (error correcting codes) to catch refresh-related bit flips.
...
ROM stores data that is **"baked in"** during manufacturing or via a special one-time programming process. It is used for boot firmware (BIOS/UEFI), system microcode, and embedded firmware. Old cartridge-based game consoles (NES, SNES, Game Boy) stored their games on ROM chips.
...
This is the heart of the lecture. Every technology we have discussed occupies a specific position in a hierarchy ordered by **distance from the CPU**. The closer to the CPU, the faster and more expensive. The further away, the slower and cheaper. This hierarchy is not a design choice — it is an economic and physical *inevitability*.
...
Our view is that of **machine code via the ISA**. We do not read individual bits; we read **instructions**, one by one, as defined by our CPU's Instruction Set Architecture. So do not imagine a horizontal tape of 1s and 0s. Imagine a **vertical tape**, 64 bits wide, broken into discrete blocks — because that is what our instructions physically *are*. Each instruction is 64 bits wide (on a 64-bit system). It is like writing an essay on a roll of toilet paper as you unroll it — each line is exactly 64 characters wide, read from top to bottom.

We group these machine instructions into **programs** and assign them to memory segments. Then we unroll a large blank section of the tape and subdivide it into the **Process Address Space** we discussed in LN10:

<PASVerticalTape />

Notice the structure: code and data sit together at the low end (top of the tape), then the heap grows downward from the code, and the stack grows upward from the bottom. The gap in between is where the two will eventually collide if you allocate too much — a **stack overflow** (the stack hits the heap) or **heap exhaustion** (the heap fills all available space).

This is the view of memory that you, as an OS developer, will optimize. Not bits, not bytes — **64-bit instruction blocks** in a vertical tape that you must partition, protect, and manage for every running process.
...
A running process does not live on just one of these rolls. Its **instructions** might be in cache (recently executed code), its **data** in RAM (active variables and heap objects), its **executable** on the SSD (the binary file), and its **logs** on the HDD (archival writes). A process *colors* parts of every roll simultaneously.
